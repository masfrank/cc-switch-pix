...
The installed skills toolbar includes a **Search** box that filters the list in real time by skill name or repository identifier.

### How to Use

1. Type keywords in the search box at the top of the installed skills list
2. Results update instantly as you type
3. Use spaces to separate multiple terms — any term matching will include the skill
...
| Field | Example |
|-------|---------|
| Skill name | `review`, `test` |
| Repository | `anthropics/claude-skills` |

> **Tip**: Search works together with source filter — apply both to narrow down results quickly.

## Batch Operations

The installed skills panel supports multi-select and batch operations, making it easy to manage many skills at once.

### Select Skills

- Click the **checkbox** on any skill card to select it
- Click **Select All** in the batch toolbar to select all currently visible (filtered) skills
- Click the **X** button to clear the selection

### Batch Toolbar

When one or more skills are selected, a batch toolbar appears with the following actions:

| Action | Description |
|--------|-------------|
| **Enable/Disable** | Click an app icon (Claude, Codex, Gemini, etc.) to toggle that app for all selected skills |
| **Change Source** | Reassign selected skills to a different repository (owner/name/branch) |
| **Delete Selected** | Uninstall all selected skills (automatic backup is created before deletion) |

### Batch Change Source

To reassign skills to a different repository:

1. Select the skills to update
2. Click **Change Source** in the batch toolbar
3. Enter the new repository owner, name, and branch
4. Optionally specify a subdirectory if skills are nested
5. Confirm — the selected skills' source info and documentation links are updated

> **Note**: Changing source does not modify the skill files themselves, only the repository association used for update detection and documentation links.

## Skill Detail View

Each installed skill has an **info button** that opens a detail dialog showing full metadata:

| Field | Description |
|-------|-------------|
| Name | Skill display name |
| Description | Skill description |
| Directory | Local installation directory name |
| Source | Repository owner/name (or "Local" for imported skills) |
| Branch | Repository branch |
| Enabled Apps | Which apps this skill is enabled for |
