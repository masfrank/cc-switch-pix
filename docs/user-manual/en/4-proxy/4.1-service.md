...
### Optional: Bridge Claude Code Through PxPipe

PxPipe can sit between Claude Code and the CC Switch local proxy for PxPipe-owned capabilities such as image compression. The request chain is:

```
Claude Code -> pxpipe -> CC Switch local proxy -> selected provider
```

Default address mapping:

| Component | Address or environment variable |
|-----------|---------------------------------|
| CC Switch local proxy | `http://127.0.0.1:15721` |
| pxpipe listen URL | `http://127.0.0.1:47821` |
| pxpipe upstream | `ANTHROPIC_UPSTREAM=http://127.0.0.1:15721` |
| Claude Code entrypoint | `ANTHROPIC_BASE_URL=http://127.0.0.1:47821` |
| pxpipe dashboard | `http://127.0.0.1:47821/` |

Start the CC Switch local proxy first, then start pxpipe with `ANTHROPIC_UPSTREAM` pointing at the CC Switch proxy address. Claude Code only needs `ANTHROPIC_BASE_URL` to point at pxpipe; CC Switch continues to own provider selection, format conversion, logging, and failover.

> **Availability**: This integration depends on CC Switch exposing PxPipe as an optional bridge. If your build does not show a bridge control, use the environment-variable wiring above manually; PxPipe should not be treated as a provider credential store inside CC Switch.
>
> **Note**: Image compression is handled by PxPipe and may be lossy. Recent turns stay as text, but exact IDs, hashes, secrets, or other strings that must remain byte-for-byte accurate should not rely on compressed image content.
