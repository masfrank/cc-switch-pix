...
### Claude + PxPipe Bridge Routing

When the PxPipe bridge is enabled, Claude Code points to pxpipe first instead of pointing directly at the CC Switch proxy:

| Location | Configuration |
|----------|---------------|
| Claude Code | `ANTHROPIC_BASE_URL=http://127.0.0.1:47821` |
| pxpipe | `ANTHROPIC_UPSTREAM=http://127.0.0.1:15721` |
| CC Switch local proxy | `http://127.0.0.1:15721` |

The full chain is `Claude Code -> pxpipe -> CC Switch local proxy -> selected provider`. Provider credentials, provider switching, logs, and failover remain managed by CC Switch; PxPipe is only an optional bridge layer.

> PxPipe image compression may be lossy. Recent turns stay as text, but exact IDs, hashes, secrets, and similar strings in images should be handled carefully.
