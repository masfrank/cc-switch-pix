...
### Import OpenAI Official Codex Provider

OpenAI Official uses Codex `auth.json` and optional `config.toml` instead of a third-party `endpoint` / `apiKey` pair. Put the `auth.json` object in `config.auth`, put the `config.toml` text in `config.config` (or an empty string when absent), Base64-encode the JSON payload, then pass it through the URL `config` query parameter.
...
```
ccswitch://v1/import?resource=provider&app=codex&name=OpenAI%20Official&configFormat=json&config=<base64-json>
```

When official login material is present in `config.auth`, it is imported into Codex `auth.json`. `config.config` is saved as that provider's Codex `config.toml`. The `enabled` parameter only controls whether the imported provider becomes current. Do not include URL-level `endpoint`, `apiKey`, or `model` for this official provider. If `endpoint` is present, the link is treated as a custom third-party Codex provider.
