...
### OpenAI Official Codex プロバイダーのインポート

OpenAI Official は、サードパーティーの `endpoint` / `apiKey` の組み合わせではなく、Codex の `auth.json` と任意の `config.toml` を使用します。`auth.json` オブジェクトを `config.auth` に入れ、`config.toml` のテキストを `config.config` に入れます（ない場合は空文字列）。その JSON payload 全体を Base64 エンコードして、URL の `config` クエリパラメータで渡します。
...
`config.auth` に公式ログイン情報が含まれる場合、その内容は Codex の `auth.json` にインポートされます。`config.config` はそのプロバイダーの Codex `config.toml` として保存されます。`enabled` パラメータは、インポートしたプロバイダーを現在のプロバイダーにするかどうかだけを制御します。この公式プロバイダーには URL レベルの `endpoint`、`apiKey`、`model` を含めないでください。`endpoint` が含まれる場合は、カスタムのサードパーティー Codex プロバイダーとして扱われます。
