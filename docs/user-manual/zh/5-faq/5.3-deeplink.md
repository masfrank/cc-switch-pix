...
### 导入 OpenAI Official Codex 供应商

OpenAI Official 使用 Codex 的 `auth.json` 和可选 `config.toml`，不是第三方供应商的 `endpoint` / `apiKey` 组合。把 `auth.json` 对象放到 `config.auth`，把 `config.toml` 文本放到 `config.config`（没有则传空字符串），然后把整个 JSON payload 做 Base64 编码，放到 URL 的 `config` 查询参数里。
...
当 `config.auth` 里包含官方登录材料时，会导入到 Codex `auth.json`。`config.config` 会作为该供应商保存的 Codex `config.toml`。`enabled` 参数只控制导入后的供应商是否成为当前供应商。这个官方供应商不要携带 URL 顶层的 `endpoint`、`apiKey` 或 `model`；如果链接里包含 `endpoint`，会按自定义第三方 Codex 供应商处理。
